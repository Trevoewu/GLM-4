...
Usage: python run_augmentation.py [--config CONFIG_FILE] [--dry-run]
"""
...
from data_augmentation import CMCCDataAugmentation
...
def check_prerequisites(config: dict) -> bool:
    """检查前置条件"""
    # 检查数据文件是否存在
    data_files = ['train_new.csv', 'dev_new.csv']
    missing_files = []
...
def run_augmentation(config: dict, dry_run: bool = False):
    ...
    try:
        ...
        # 处理验证集（如果需要）
        if os.path.exists("dev_new.csv") and output_config.get('balanced_dev_file'):
            logging.info("处理验证集...")
            augmentor.create_balanced_dataset(
                original_csv="dev_new.csv", 
